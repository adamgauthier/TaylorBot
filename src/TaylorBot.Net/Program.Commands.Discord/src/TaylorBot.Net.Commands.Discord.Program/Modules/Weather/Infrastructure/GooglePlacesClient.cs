...
namespace TaylorBot.Net.Commands.Discord.Program.Modules.Weather.Infrastructure
{
    public class GooglePlacesClient : ILocationClient
    {
        ...
        {
            var qs = QueryString.Create(new[] {
                new KeyValuePair<string, string>("key", _options.CurrentValue.GoogleApiKey),
                new KeyValuePair<string, string>("inputtype", "textquery"),
                new KeyValuePair<string, string>("fields", "formatted_address,geometry/location"),
                new KeyValuePair<string, string>("input", search),
            }).ToUriComponent();
