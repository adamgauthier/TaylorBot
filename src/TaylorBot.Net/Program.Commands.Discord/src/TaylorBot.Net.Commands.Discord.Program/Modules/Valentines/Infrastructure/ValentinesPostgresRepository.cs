...
namespace TaylorBot.Net.Commands.Discord.Program.Modules.Valentines.Infrastructure
{
    public class ValentinesPostgresRepository : IValentinesRepository
    {
        private readonly PostgresConnectionFactory _postgresConnectionFactory;

        public ValentinesPostgresRepository(PostgresConnectionFactory postgresConnectionFactory)
        {
        ...
        public async ValueTask<RoleObtained?> GetRoleObtainedByUserAsync(IGuildUser user)
        {
            await using var connection = _postgresConnectionFactory.CreateConnection();
        ...
        public async ValueTask<DateTimeOffset> SpreadRoleAsync(IGuildUser fromUser, IGuildUser toUser)
        {
            await using var connection = _postgresConnectionFactory.CreateConnection();
            ...
            var acquiredAt = await connection.QuerySingleAsync<DateTime>(
                ...
                new
                {
                    ...
                    ToUserFullUsername = $"{toUser.Username}#{toUser.Discriminator}",
                    FromUserId = fromUser.Id.ToString(),
                    FromUserFullUsername = $"{fromUser.Username}#{fromUser.Discriminator}",
                }
            );
        ...
        public async ValueTask<IReadOnlyList<RoleObtained>> GetRoleObtainedFromUserAsync(IGuildUser user)
        {
            await using var connection = _postgresConnectionFactory.CreateConnection();
        ...
        public async ValueTask<IReadOnlyList<RoleObtained>> GetAllReadyAsync(ValentinesConfig config)
        {
            ...
            var records = await connection.QueryAsync<RoleObtainedDto>(
                ...
                new
                {
                    SpreadLimit = config.SpreadLimit,
